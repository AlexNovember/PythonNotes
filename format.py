...
class Notes:
    ...
    def __str__(self):
        return 'id ' + str(self.note_id)  + ' | ' + datetime.datetime.strftime(self.date, '%d.%m.%Y') + ' | ' + self.title + (
                    10 - len(self.title)) * ' ' + ' | ' + self.text
    ...
    def to_csv(self):
        return str(self.note_id) + ' | ' + self.title + ' | ' + self.text + ' | ' + \
            datetime.datetime.strftime(self.date, '%d.%m.%Y') + ' | ' + datetime.time.strftime(self.time,'%H:%M:%S') + '\n'
